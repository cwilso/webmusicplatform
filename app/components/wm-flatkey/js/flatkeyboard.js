/**
 *  flatKeyboard.js v1.1.2 by @ryoyakawai
...
var FlatKeyboard = function(elem, octave) {
    this.touchSupport = ('ontouchstart' in window);
    this.drawKeys=[];
    this.drawCtrls=[];
    this.mouseClick=false;
    
    if(typeof octave=="undefined" || octave<1 || octave>7 ) {
        octave=2;
    }

    this.octave={"draw":octave, "now":3, "min":1, "max":7};
    this.drawSize={
    ...
    function updateNoteon(keyOn) {
        for(var i=0; i<this.key.key2Itnl.length; i++) {
            // 重複鳴り制御
            // noteOnStatus[i]===false > 鳴ってない
            //  >> じゃあ鳴らす
            // ELSE noteOnStatus[i]===false > 鳴ってる
            // 　&& typeof keyOn[i]=="undefined" 鳴らせって言ってない
            //  >> じゃあ止める
    ...
    function updateKey(event) {
        ...
        for(var i=0; i<pos.length; i++) {
            ...
            if(key.num=="undefined" || key.type=="undefined") {
                break;
            }
            // conver to Key Name
            ...
            if(key.type=="key" && key.num!="undefined") {
                ...
                this.drawKeys[key.num].on={"status":true, "velocity": 0x64};
    ...
    function updateCtrl(key) {
        if(key.label.match(/octave/)!=null) {
            if(key.label.match(/Plus/)!=null) {
                if(this.octave.max>=parseInt(this.octave.now)+parseInt(this.octave.draw)) {
                    this.octave.now++;
                }
            }
    ...
    function allDispNoteOff() {
        for(var i=0; i<this.drawKeys.length; i++) {
            this.drawKeys[i].on={"status": false, "velocity":0};
        }
...
FlatKeyboard.prototype={
    ...
    onmessage: function(midiMsg) {
        ...
        if(data.msg.substr(0, 1).match(/^8$|^9$/)==null) {
            return;
        }
        ...
        if(this.octave.now<=tOct && this.octave.now+2>=tOct) {
            for(var i=0; i<this.drawKeys.length; i++) {
                if(tko == (this.drawKeys[i].label.split(":")).join("")) {
                    var velocity=data.raw[2];
                    if(data.msg.substr(0, 1)=="8") {
                        this.drawKeys[i].on={"status": false, "velocity":0};
                    }
                    if(data.msg.substr(0, 1)=="9") {
                        this.drawKeys[i].on={"status": true, "velocity":velocity};
                    }
                }
            }
        }
    ...
    draw: function() {
        ...
        for(var i=0; i<this.drawKeys.length; i++) {
            ...
            if(this.drawKeys[i].on.status==true){
                var v=Math.ceil((this.drawKeys[i].on.velocity/127)*10)/10;
                this.context.fillStyle="rgba(249, 137, 124, "+v+")";
            }
